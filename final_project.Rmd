...
```{r}
complex_model <- lm(`Barking Level` ~ `Watchdog/Protective Nature` + `Openness To Strangers` + `Affectionate With Family` + `Good With Young Children` + `Good With Other Dogs` + `Playfulness Level` + `Adaptability Level` + `Trainability Level` + `Energy Level` + `Mental Stimulation Needs`, data = breed_traits)
...
complex_model %>%
  augment() %>%
  select(`Openness To Strangers`, `Affectionate With Family`, `Good With Young Children`, `Playfulness Level`, `Adaptability Level`, `Trainability Level`, `Energy Level`, `Mental Stimulation Needs`, cooks.dist = .cooksd) %>%
...
```{r}
# Calculate VIF
vif(complex_model) # Every one of our variables are well below zero, so we have encountered not high multicorrealianity in our model. Therefoe we will not drop any of our predictor from the more complex model.
```
...
glance(null_model) # Adjusted R squaired =  AIC = 
```
...
glance(complex_model) # Adjusted R squaired = AIC = 
```

Compare the two models.

```{r}
anova(null_model,complex_model) # F-statistic =  p-value = 
```
## Discussion
